...
__m512i one             = _mm512_ternarylogic_epi32(zero, zero, zero, 0xFF);
...
for (i = n; i < b_len; i++) {
    ...
    // hidden (1c). Check if last iter's carry bits were mispredicted.
    if (__builtin_expect(failed, 0)) {
        // todo: mispredict cleanup.
    }
    ...
    __mmask16 eq_check    = _mm512_test_cmpeq_epi32_mask(eq, zero); // hidden (3c). Check if carry bits for this iter's sum were good.
    __mmask16 vp_check    = _mm512_test_cmpeq_epi32_mask(vp, ones); // hidden (3c). Check if carry bits for this iter's sum were good.
